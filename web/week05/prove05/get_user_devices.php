<?php

require "db_connect.php";
$db = getDatabase();

$username = $_GET['username'];

$user_row = $db->prepare("SELECT user_id FROM users WHERE username='$username' LIMIT 1");
$user_row->execute();

$user = $user_row->fetch(PDO::FETCH_ASSOC);

$user_id = $user['user_id'];

$location_row = $db->prepare("SELECT location_id, location_name FROM locations WHERE user_id='$user_id'");
$location_row->execute();
$floor_row = $db->prepare("SELECT floor_id, location_id, floor_name FROM floors WHERE user_id='$user_id'");
$floor_row->execute();
$device_row = $db->prepare("SELECT device_id, floor_id, device_description, device_notes, device_address, device_canFrame FROM devices WHERE user_id='$user_id'");
$device_row->execute();

while ($location = $location_row->fetch(PDO::FETCH_ASSOC)) {
  $location_id = $location['location_id'];
  $location_name = $location['location_name'];


}

?>